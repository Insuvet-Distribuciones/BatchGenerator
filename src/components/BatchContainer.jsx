...
function BatchContainer({ batch }) {
  ...
  const currentDateFormat = new Date().toISOString().slice(0, 10).replace(/-/g, "/");
...
<div className="font-poppins  flex flex-col gap-2 p-2  text-[8px] leading-tight pdf-cell">
  ...
  <div className="flex flex-col items-center">
    <p>Peso Neto al empacar <span>{batch.totalWeight}</span></p>
    <p>Registro ICA - <span>{batch.itemICAId}</span></p>
  </div>
  <div className="text-center">
    <h2 className="text-[15px] font-bold">INDICACIONES DEL PRODUCTO</h2>
    <p className="break-words text-[10px] whitespace-pre-wrap">{batch.indications}</p>
  </div>
  <div className="text-center">
    <h2 className="text-[15px] font-bold">INGREDIENTES</h2>
    <p className="font-semibold text-[12px] break-words ">{batch.ingredients}</p>
  </div>
  ...
  <div className="flex justify-between">
    <div>
      <p className="text-[7px]">Fabricado por: Juan Carlos Auza Fernandez</p>
      <p className="text-[7px]">Dirección: Calle 2 Sur # 5 A 53 Neiva, Huila</p>
